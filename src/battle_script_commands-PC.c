...
static void Cmd_attackcanceler(void)
{
    ...
    else if (gSpecialStatuses[gBattlerTarget].vaporizeStopsWater)
    {
        gSpecialStatuses[gBattlerTarget].vaporizeStopsWater = FALSE;
        gLastUsedAbility = ABILITY_VAPORIZE;
        BattleScriptPushCursor();
        gBattlescriptCurrInstr = BattleScript_VaporizeStopsWater;
        RecordAbilityBattle(gBattlerTarget, gLastUsedAbility);
    }
...
u32 GetTotalAccuracy(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect)
{
    ...
    case ABILITY_KEEN_EYE:
        calc = (calc * 115) / 100; // 1.15 keen eye boost
        break;
