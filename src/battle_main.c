...
const struct TrainerMoney gTrainerMoneyTable[] =
{
    ...
    {TRAINER_CLASS_TUBER_F, 4},
    {TRAINER_CLASS_TUBER_M, 4},
    {TRAINER_CLASS_SIS_AND_BRO, 5},
    {TRAINER_CLASS_COOLTRAINER, 12},
    {TRAINER_CLASS_HEX_MANIAC, 8},
        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            if (!(gBattleControllerExecFlags & ((gBitTable[battler]) | (0xF << 28) | (gBitTable[battler] << 4) | (gBitTable[battler] << 8) | (gBitTable[battler] << 12))))
            {
                ...
                case B_ACTION_USE_ITEM:
                    if (FlagGet(B_FLAG_NO_BAG_USE) || (FlagGet(FLAG_HARDMODE_BAG_DISABLE) && gBattleTypeFlags & (BATTLE_TYPE_TRAINER | BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_ARENA | BATTLE_TYPE_MULTI | BATTLE_TYPE_PALACE)))
                    {
